...
if [ ! -f blobs/"$deviceid"-"$version".shsh2 ]; then
    ...
    if [[ ! "$@" == *"--no-install"* ]]; then
        ...
        "$dir"/sshpass -p 'alpine' ssh -o StrictHostKeyChecking=no -p2222 root@localhost "/bin/rm -rf /usr/local/bin/loader.app/Info.plist /usr/local/bin/loader.app/Base.lproj /usr/local/bin/loader.app/PkgInfo"
        sleep 1
